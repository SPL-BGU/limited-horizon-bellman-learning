...
class Cube3State(State):
    ...
    def __hash__(self):
        if self.hash is None:
            try:
                self.hash = hash(self.colors.tostring())  # Old NumPy (<2.0)
            except AttributeError:
                self.hash = hash(self.colors.tobytes())   # New NumPy (>=2.0)
    ...
    def __repr__(self):
        return f"Cube3State(colors={self.colors.tolist()})"
...
class Cube3(Environment):
    ...
    def generate_states(self, num_states: int, backwards_range: Tuple[int, int]) -> Tuple[List[Cube3State], List[int]]:
        ...
        # Scrambles
        # scramble_nums: np.array = self.rand_gen.choice(scrambs, num_states)
        ...
        while np.any(moves_lt):
            ...
            # idxs: np.ndarray = self.rand_gen.choice(idxs, subset_size)
            idxs: np.ndarray = np.random.choice(idxs, subset_size)

            # move: int = self.rand_gen.integers(low=0, high=num_env_moves)
