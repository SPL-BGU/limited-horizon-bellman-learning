...
class GBFS:
    def __init__(self, states: List[State], env: Environment, eps: Optional[List[float]] = None, train_method: str = ""):
        self.curr_states: List[State] = states
        self.env: Environment = env
        self.train_method: str = train_method
    ...
    def _move(self, heuristic_fn: Callable) -> None:
        ...
        ctg_backups, ctg_next_p_tcs, states_exp = search_utils.bellman(states, heuristic_fn, self.env, self.train_method)
...
def gbfs_test(num_states: int, back_max: int, env: Environment, heuristic_fn: Callable,
              max_solve_steps: Optional[int] = None, train_method: str = ""):
    # get data
    ...
    # Solve with GBFS
    gbfs = GBFS(states, env, eps=None, train_method=train_method)
